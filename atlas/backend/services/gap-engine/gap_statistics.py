...
def run_hypothesis_tests(gap_df: pd.DataFrame) -> list[dict]:
    ...
    def h1():
        ...
        _, p_val = stats.binom_test(len(up_close), len(upper_third), 0.60, alternative="greater") if hasattr(stats, 'binom_test') else stats.binomtest(len(up_close), len(upper_third), 0.60, alternative="greater").pvalue, None
